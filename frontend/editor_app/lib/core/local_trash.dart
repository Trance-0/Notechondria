...
/// Client-side recycle-bin for drafts and courses that were successfully
/// synced to the cloud. After a local item is promoted to the cloud we
/// move it here (tagged with `trashed_at` and the server-side id/uuid)
/// instead of discarding it, so the user can restore a local working
/// copy if the cloud version later turns out wrong, corrupted, or
/// misleading.
///
/// Auto-pruned at load time after [_LocalAppStore._trashTtlDays] days
/// (see local_store.dart). The state fields live on `_AppShellState`
/// so they can be reassigned via `setState`; this extension carries
/// the read/write logic so app_shell.dart can stay lean.
extension _AppShellLocalTrashX on _AppShellState {
  ...
  /// Called after a local draft has been successfully promoted to the
  /// cloud. Moves the draft into the recycle bin carrying its
  /// `trashed_at` timestamp + the server note id / uuid that replaced
  /// it, so the user can restore if the cloud copy later turns out
  /// wrong or the sync response was misleading.
  ...
  /// Copies a recycle-bin entry back to `_localDrafts` and drops it
  /// from the trash bucket. The restored draft gets a fresh local
  /// `id` so it never collides with the still-existing cloud copy.
  /// The user keeps both: the cloud note AND a local draft they can
  /// compare / edit / re-sync.
  Future<ActionFeedback> _restoreTrashedDraft(
    ...
    if (raw is! Map) {
      return const ActionFeedback(
        message: 'Draft not restored: '
            'Editor.LocalStore/restore_trashed_draft \u2014 '
    ...
    return ActionFeedback(
      message: 'Draft restored: Editor.LocalStore/restore_trashed_draft \u2014 '
          "'$title' is back under Drafts. The cloud copy was not touched.",
    );
  }
  ...
  Future<ActionFeedback> _restoreTrashedCourse(
    ...
    if (raw is! Map) {
      return const ActionFeedback(
        message: 'Category not restored: '
            'Editor.LocalStore/restore_trashed_course \u2014 '
    ...
    return ActionFeedback(
      message:
          'Category restored: Editor.LocalStore/restore_trashed_course \u2014 '
          "'$title' is back. The cloud copy was not touched.",
    );
  }

  /// Shows a bottom-sheet listing every entry currently in the local
  /// recycle bin with a Restore action per entry. Re-opens itself
  /// after each restore so the user can process multiple entries in
  /// one session.
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      ...
                      'successful cloud sync. Restore to bring one back as a '
                      'local copy without touching the cloud version.',
