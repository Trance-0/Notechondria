...
class _AppShellState extends State<AppShell> {
  ...
  // Local recycle-bin helpers (`_moveDraftToLocalTrash`,
  // `_moveCourseToLocalTrash`, `_restoreTrashedDraft`,
  // `_restoreTrashedCourse`, `_openLocalRecycleBinDialog`,
  // `_persistLocalTrashed*`, `_formatTrashedAt`) live in
  // `core/local_trash.dart` as an extension on `_AppShellState` so
  // this file stays under the AGENTS.md \u00a71.5 1000-line ceiling.

  /// Tiny `setState` wrapper so extensions defined outside the class
  /// body can still trigger a rebuild. `setState` itself is
  /// protected and invisible to extensions, even those declared in
  /// the same library. Keep the closure empty \u2014 extensions mutate
  /// the state fields directly before calling this.
  void _trashRefresh() {
    if (mounted) setState(() {});
  }
