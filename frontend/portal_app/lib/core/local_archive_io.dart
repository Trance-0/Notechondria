...
/// Local archive (.nchron) export + import helpers for the portal app.
...
extension _AppShellLocalArchiveX on _AppShellState {
  /// Exports local portal data as a `.nchron` package.
  Future<void> _exportLocalArchive() async {
    try {
      final username = _profile?['username']?.toString() ?? 'portal';
      final suggestedName = 'notechondria-portal-$username.nchron';
      ...
      showMessage(
        'Local data exported: '
        'Portal.LocalStore/export_zip — $suggestedName written '
        '(${archiveBytes.length} bytes).',
      );
      log(
        level: DebugLogLevel.info,
        source: 'Portal.LocalStore/export_zip',
        message: 'Local data exported: '
            'Portal.LocalStore/export_zip — '
      ...
      showMessage(
        ...
        'Portal.LocalStore/export_zip — $cause.',
      );
      log(
  ...
  Future<void> _restoreFromLocalImport() async {
    try {
      ...
      if (legacy != null) {
        ...
        log(
          level: DebugLogLevel.info,
          source: 'Portal.LocalStore/restore_from_import',
          message: 'Legacy .env imported: '
              'Portal.LocalStore/restore_from_import — '
      ...
      showMessage(
        ...
        'Portal.LocalStore/restore_from_import — $cause.',
      );
      log(
        level: DebugLogLevel.error,
        source: 'Portal.LocalStore/restore_from_import',
