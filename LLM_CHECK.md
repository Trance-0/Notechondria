...
- Changed the frontend Docker build to run Flutter web build steps as root, which matches Flutter's official web deployment flow of generating `build/web` and serving it with a normal web server, and avoids SDK cache permission failures under `/sdks/flutter`.
